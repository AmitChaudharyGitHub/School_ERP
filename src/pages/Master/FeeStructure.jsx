...
import { ClipboardList, IndianRupee, Pencil, School, Trash2, WalletCards, LayoutDashboard, Plus, RotateCcw, Search, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Menu } from "lucide-react";

const FeeStructure = () => {
  const feeData = [
    { id: 1, className: "Class 5", academicYear: "2024-25", tuition: 25000, admission: 3000, exam: 2000, transport: 6000, other: 2000, total: 38000, status: "Active" },
    { id: 2, className: "Class 6", academicYear: "2024-25", tuition: 26000, admission: 3000, exam: 2000, transport: 6000, other: 2000, total: 39000, status: "Active" },
    { id: 3, className: "Class 7", academicYear: "2024-25", tuition: 27000, admission: 3000, exam: 2500, transport: 6000, other: 2500, total: 41000, status: "Active" },
    { id: 4, className: "Class 8", academicYear: "2024-25", tuition: 28000, admission: 3000, exam: 2500, transport: 6500, other: 2500, total: 42500, status: "Active" },
    { id: 5, className: "Class 9", academicYear: "2024-25", tuition: 29000, admission: 3000, exam: 2500, transport: 6500, other: 2500, total: 43500, status: "Draft" },
    { id: 6, className: "Class 10", academicYear: "2024-25", tuition: 30000, admission: 3000, exam: 3000, transport: 6500, other: 2500, total: 45000, status: "Active" },
  ];

  const actionButtonStyle = { width: "28px", height: "28px", padding: 0, borderRadius: "6px" };
  const money = useCallback((value) => `Rs. ${value.toLocaleString("en-IN")}`, []);

  const columns = [
    { key: "className", label: "Class", minWidth: "100px" },
    { key: "academicYear", label: "Academic Year", minWidth: "120px" },
    { key: "tuition", label: "Tuition Fee", minWidth: "110px", render: (row) => money(row.tuition) },
    { key: "admission", label: "Admission Fee", minWidth: "130px", render: (row) => money(row.admission) },
    { key: "exam", label: "Exam Fee", minWidth: "100px", render: (row) => money(row.exam) },
    { key: "transport", label: "Transport Fee", minWidth: "130px", render: (row) => money(row.transport) },
    { key: "other", label: "Other Fee", minWidth: "100px", render: (row) => money(row.other) },
    { key: "total", label: "Total", minWidth: "100px", render: (row) => <strong>{money(row.total)}</strong> },
    { key: "status", label: "Status", minWidth: "100px", render: (row) => <span className={`badge ${row.status === "Active" ? "bg-success" : "bg-warning text-dark"}`}>{row.status}</span> },
    {
      key: "actions",
      label: "Actions",
      minWidth: "100px",
      render: (row) => (
        <div className="d-flex gap-2">
          <button className="btn btn-sm btn-outline-primary d-inline-flex align-items-center justify-content-center" style={actionButtonStyle} title={`Edit ${row.className}`} aria-label={`Edit ${row.className}`}>
            <Pencil size={15} />
          </button>
          <button className="btn btn-sm btn-outline-danger d-inline-flex align-items-center justify-content-center" style={actionButtonStyle} title={`Delete ${row.className}`} aria-label={`Delete ${row.className}`}>
            <Trash2 size={15} />
          </button>
        </div>
      ),
  ...
  const summaryCards = [
    { title: "Standards", value: "6", icon: School, color: "#2563eb", background: "#eff6ff" },
    { title: "Active Structure", value: "6", icon: ClipboardList, color: "#ea580c", background: "#fff7ed" },
    { title: "Lowest Fee", value: "Rs. 35K", note: "Class 5", icon: WalletCards, color: "#7c3aed", background: "#f5f3ff" },
    { title: "Highest Fee", value: "Rs. 45K", note: "Class 10", icon: WalletCards, color: "#7c3aed", background: "#f5f3ff" },
    { title: "Avg. Annual Fee", value: "Rs. 41.8K", note: "Class 5-10", icon: IndianRupee, color: "#16a34a", background: "#f0f4f4" },
  ];
  const filters = [
    { key: "academicYear", label: "Academic Year", options: ["2024-25"] },
    { key: "className", label: "Standard", options: ["Class 5", "Class 6", "Class 7", "Class 8", "Class 9", "Class 10"] },
    { key: "status", label: "Status", options: ["Active", "Draft"] },
  ];
  const primaryAction = { label: "Add Fee Structure" };
  const tableTitle = "Fee Structure";
  ...
  const filteredRows = useMemo(() => {
    return feeData.filter((row) => {
      ...
      return matchesFilters && matchesColumnFilters;
    });
  }, [filterValues, filters, feeData, columnFilters]);
        ...
        <div className="card-body">
          <div className="d-flex align-items-end gap-2 flex-wrap">
            ...
            <div className="filter-actions d-flex align-items-end gap-2" style={{ flex: "0 0 72px" }}>
              ...
              <button className="btn btn-outline-secondary d-flex align-items-center justify-content-center" aria-label="Reset" title="Reset" onClick={resetAllFilters}><RotateCcw size={16} /></button>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="table-shell position-relative">
              ...
              {showColumnMenu && (
                <div className="position-absolute bg-white border rounded-3 shadow-sm p-2 text-start" style={{ width: "200px", top: "36px", right: "8px", zIndex: 20 }}>
                  ...
                  <button type="button" className="btn btn-light btn-sm w-100 text-start mb-2" onClick={resetAllFilters}>
                    Clear All Filters
                  </button>
